#include "Operation_HarvestTimingSOH.h"

#include <PlatformUtils/PlatformUtils.h>

const uint32_t CLOCK_STATUS_INPUTS_POOL_SIZE_DEFAULT = 1023;

Operation_HarvestTimingSOH::Operation_HarvestTimingSOH(const std::string& libPath,
  const sHarvestParams& params,
  const std::shared_ptr<IClient>& client,
  const std::shared_ptr<IProgressDelegate>& progressDelegate)
: IOperation("harvest-timing-soh")
, m_libPath(libPath)
, m_params(params)
, m_client(client)
...
bool Operation_HarvestTimingSOH::_executeImpl(const std::shared_ptr<IHasher>& hasher,
  const std::shared_ptr<ILogger>& logger)
{
  ...
  m_headFilter = graphBuilder.buildClockStatusHaverstingChain(this, pLibrary, m_params,
    CLOCK_STATUS_INPUTS_POOL_SIZE_DEFAULT, m_client, m_progressDelegate);
...
bool Operation_HarvestTimingSOH::_stopImpl()
{
  __DEV_CALLSTACK_FUNC__;
  if (m_headFilter != nullptr) {
