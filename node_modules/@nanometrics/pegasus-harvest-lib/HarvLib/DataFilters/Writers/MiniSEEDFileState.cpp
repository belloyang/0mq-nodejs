...
MiniSEEDFileState::MiniSEEDFileState(const std::string& name, const uint32_t nChannels, const std::shared_ptr<ILogger>& logger)
  : m_name(name)
  , m_nChannelsToFill(nChannels)
  , m_recordOffset(0ull)
  , m_nRecords(0ul)
  , m_context(nullptr)
  , m_logger(logger)
...
void MiniSEEDFileState::createContext(const std::string& workingDir, const uint32_t cacheSize)
{
  __DEV_CALLSTACK_FUNC__;
  ...
  m_context = std::make_shared<BufferedIOContext>(fileContext,
    0 /*no cache for reading as we are only writing to miniSEED*/,
    cacheSize,
    m_logger);
}

void MiniSEEDFileState::finishRecord(const uint32_t recordLen)
{
  __DEV_CALLSTACK_FUNC__;
  m_nRecords++;
...
uint8_t MiniSEEDFileState::finishChannel()
{
  __DEV_CALLSTACK_FUNC__;
  return --m_nChannelsToFill;
}
