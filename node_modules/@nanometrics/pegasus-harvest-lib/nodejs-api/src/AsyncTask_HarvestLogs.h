#ifndef AsyncTask_HarvestLogs_h__
#define AsyncTask_HarvestLogs_h__

#include "IAsyncTask.h"
#include <Operations/operations.h>

class AsyncTask_HarvestLogs : public IAsyncTask
{
public:
  AsyncTask_HarvestLogs(const std::string& libPath,
    const std::string& paramsJSON,
    const float updateStep,
    const std::shared_ptr<IHasher>& hasher,
    const std::shared_ptr<ILogger>& logger,
    const std::shared_ptr<IClient>& client);
  virtual ~AsyncTask_HarvestLogs() = default;
...
#endif //AsyncTask_HarvestLogs_h__
