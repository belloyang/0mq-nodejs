#include "AsyncTask_HarvestLogs.h"
...
AsyncTask_HarvestLogs::AsyncTask_HarvestLogs(const std::string& libPath,
  const std::string& paramsJSON,
  const float updateStep,
  const std::shared_ptr<IHasher>& hasher,
  const std::shared_ptr<ILogger>& logger,
  const std::shared_ptr<IClient>& client)
  : IAsyncTask("harvest_logs", hasher, std::make_shared<JSONLogger>(this, logger))
  , m_libPath(libPath)
...
void AsyncTask_HarvestLogs::_runImpl(const std::shared_ptr<IContext>& context)
{
  __DEV_CALLSTACK_FUNC__;
  ...
  if (HarvestParamsParser(m_logger).parse(m_paramsJSON, params))
  {
    ...
    m_operation = std::make_shared<Operation_HarvestLogs>(m_libPath, params, m_client,
...
bool AsyncTask_HarvestLogs::_stopImpl()
{
  __DEV_CALLSTACK_FUNC__;
