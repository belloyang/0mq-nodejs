#include "AsyncTask_HarvestTimingSOH.h"
...
AsyncTask_HarvestTimingSOH::AsyncTask_HarvestTimingSOH(const std::string& libPath,
  const std::string& paramsJSON,
  const float updateStep,
  const std::shared_ptr<IHasher>& hasher,
  const std::shared_ptr<ILogger>& logger,
  const std::shared_ptr<IClient>& client)
  : IAsyncTask("harvest_timing_soh", hasher, std::make_shared<JSONLogger>(this, logger))
  , m_libPath(libPath)
...
void AsyncTask_HarvestTimingSOH::_runImpl(const std::shared_ptr<IContext>& context)
{
  __DEV_CALLSTACK_FUNC__;
  ...
  if (HarvestParamsParser(m_logger).parse(m_paramsJSON, params))
  {
    ...
    m_operation = std::make_shared<Operation_HarvestTimingSOH>(m_libPath, params, m_client,
...
bool AsyncTask_HarvestTimingSOH::_stopImpl()
{
  __DEV_CALLSTACK_FUNC__;
